...
const $__defun = (state, name) => (...args) => {
  ...
  const car = $$first(...args);
  const fn = $__lambda(state, name)(...args.slice(1)); // TODO: overloading
...
const $comp$__defun = (state, name) => (...args) => {
  ...
  Object(_errors__WEBPACK_IMPORTED_MODULE_4__["checkParamsLength"])('$$__defun', args, 3);
  const car = $$first(...args);
  const fn = $comp$__lambda(state, name)(...args.slice(1)); // TODO: overloading
...
const $__defmacro = (state, name) => (...args) => {
  ...
  const fn = fArgs => (...aArgs) => {
    return $__scope(state, name, capturedScopes)(false, false, [[state.config.reservedNames.self, fn], ...fArgs.map((x, index) => [x.symbol, Object(_ast__WEBPACK_IMPORTED_MODULE_1__["quote"])(state, lastIsSpread && index === fArgs.length - 1 ? aArgs.slice(index) : aArgs[index])])], ...fnBody);
  };
