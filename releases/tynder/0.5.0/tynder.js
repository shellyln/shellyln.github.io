...
function compile(s) {
  ...
  lisp.setGlobals({
    ...
    '@constraint': (name, args) => ty => _operators__WEBPACK_IMPORTED_MODULE_7__["withConstraint"](name, args)(ty),
...
function generateCSharpCodeRepeated(ty, ctx) {
  return `${ty.repeated.typeName ? formatTypeName(ty.repeated, ctx, ty.repeated.typeName) : ty.repeated.kind === 'primitive' ? generateCSharpCodePrimitive(ty.repeated, ctx) : ty.repeated.kind === 'primitive-value' ? generateCSharpCodePrimitiveValue(ty.repeated, ctx) : 'object'}[]`;
}
...
function generateGraphQlCodeRepeated(ty, ctx) {
  return `[${ty.repeated.typeName ? formatTypeName(ty.repeated, ctx, ty.repeated.typeName) : generateGraphQlCodeInner(ty.repeated, false, ctx, false)}${ty.repeated.kind === 'optional' ? '' : '!'}]`;
}
...
function generateGraphQlCodeOneOf(ty, ctx, isUnion) {
  ...
  if (filtered.length === 1 && ty.oneOf.length !== 1) {
    return filtered[0].typeName ? filtered[0].typeName : generateGraphQlCodeInner(filtered[0], false, ctx, false);
  } else {
    if (isUnion) {
...
function generateProto3CodeRepeated(ty, ctx) {
  return `repeated ${ty.repeated.typeName ? formatTypeName(ty.repeated, ctx, ty.repeated.typeName) : generateProto3CodeInner(ty.repeated, false, ctx)}`;
}
