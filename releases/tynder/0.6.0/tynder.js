...
const exportedDef = trans(tokens => [[{
  symbol: 'export'
}, tokens[0]]])(erase(seq('export'), qty(1)(commentOrSpace)), first(constDef, internalDef, input => declareVarStatement(input), err('exportedDef: Unexpected token has appeared.')));
...
const declareVarStatement = trans(tokens => [[{
  symbol: 'passthru'
}, tokens[0]]])(cat(seq('declare'), // TODO: [export] declare (var|let|const) varName = ... // <- pass-thru
//       [export] [declare] type typeName = ...         // <- NOT pass-thru
//       [export] [declare] [const] enum = ...          // <- NOT pass-thru
qty(1)(commentOrSpace), first(seq('var'), seq('let'), seq('const'), err('declareVarStatement: Unexpected token has appeared. Expect "var|let|const".')), qty(1)(commentOrSpace), cat(repeat(notCls(';'))), first(ahead(seq(';')), err('declareVarStatement: Unexpected token has appeared. Expect ";".')), cls(';')));
...
function parseExternalDirective(s) {
  const z = externalTypeDef(Object(fruitsconfits_modules_lib_types__WEBPACK_IMPORTED_MODULE_0__["parserInput"])(s, {
    /* TODO: set initial state to the context */
  }));
...
function compile(s) {
  ...
  const external = (...names) => {
    for (const name of names) {
      ...
      if (typeof name === 'string') {
        ty = def(name, _operators__WEBPACK_IMPORTED_MODULE_7__["primitive"]('any'));
      } else {
        ty = def(name[0], name[1] ? name[1] : _operators__WEBPACK_IMPORTED_MODULE_7__["primitive"]('any'));
      }
